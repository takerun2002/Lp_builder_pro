...
**「内田式リサーチメソッドをAIで自動化し、売れるコンセプトを導き出すリサーチエージェント」**
...
```typescript
interface UchidaResearchResult {
  // 市場分析
  market: {
...
## 概要
内田式コンセプト作成メソッドをベースにした、AIリサーチエージェントを実装してください。
...
│
├── lib/research/
│   ├── types.ts                   # 型定義（上記UchidaResearchResult）
...
### 1. 競合分析が最重要
内田式では「競合のセールスレターはリサーチの集大成」とされています。
