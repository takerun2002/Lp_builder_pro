...
たけるん式コンセプト作成メソッドをAI自動化し、ターゲットの悩み収集→競合分析→売れるコンセプト提案を行うリサーチエージェントを実装します。
...
- たけるん式ポイント: 「平均5万のチャンネルで49万再生=刺さっている」
```
...
#### Task 4.1: たけるん式コンセプトジェネレーター
```
ファイル: src/lib/research/concept-generator.ts

機能:
- 収集したデータを統合
- たけるん式6ステップに基づきコンセプト生成
...
### 1. 競合分析が最重要
> 「競合のセールスレターはリサーチの集大成」（たけるん式）

競合のセールスレターを徹底的に分析し、コンセプトを抽出することを最優先してください。

### 2. 「点」を大量に集める
> 「頭の中にないものは出てこない」（たけるん式）
...
- `docs/research_agent_uchida_spec.md` - たけるん式対応仕様書
- `docs/research_agent_implementation.md` - 実装ガイド
- `/Users/okajima/Downloads/たけるん式売れるコンセプト作成.txt` - たけるん式メソッド原文
- `/Users/okajima/Downloads/たけるん式リサーチ方法.txt` - リサーチ方法原文
